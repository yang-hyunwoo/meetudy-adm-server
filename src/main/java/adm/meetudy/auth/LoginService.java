...
import static adm.meetudy.constant.member.MemberProviderTypeEnum.*;
...
public class LoginService implements UserDetailsService {
    ...
    public UserDetails loadUserByUsername(String email) throws UsernameNotFoundException {
        Member member = memberRepository.findByEmailAndProvider(email, NORMAL)
                .orElseThrow(
                        () -> new InternalAuthenticationServiceException(LG_MEMBER_ID_PW_INVALID.getMessage())
                );
